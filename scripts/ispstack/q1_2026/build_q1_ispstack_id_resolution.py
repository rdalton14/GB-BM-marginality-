...
CANDIDATES_PATH = BASE_DIR / "marginal_candidates_sp_q1_2026.parquet"
ACTION_PATH = BASE_DIR / "marginal_action_sp_q1_2026.parquet"
...
OUT_REGISTER = PROJECT_ROOT / "data" / "interim" / "ispstack" / "ispstack_id_resolution_q1_2026.csv"
OUT_REGISTER_PARQUET = OUT_REGISTER.with_suffix(".parquet")
OUT_REVIEW = PROJECT_ROOT / "data" / "diagnostics" / "audits" / "ispstack_id_resolution_q1_2026_review_queue.csv"
OUT_AUDIT = PROJECT_ROOT / "data" / "diagnostics" / "audits" / "ispstack_id_resolution_q1_2026_audit.csv"
OUT_SUMMARY = PROJECT_ROOT / "data" / "diagnostics" / "audits" / "ispstack_id_resolution_q1_2026_summary.json"
OUT_ACTION_APPLIED = BASE_DIR / "marginal_action_sp_q1_2026_resolved.parquet"
OUT_CANDIDATES_APPLIED = BASE_DIR / "marginal_candidates_sp_q1_2026_resolved.parquet"
OUT_ACTION_FINAL = BASE_DIR / "marginal_action_sp_q1_2026_identity_final.parquet"
OUT_FINAL_AUDIT = PROJECT_ROOT / "data" / "diagnostics" / "audits" / "ispstack_id_resolution_q1_2026_identity_final_audit.csv"
...
def build_resolution_register(usage: pd.DataFrame, bmu_ref: pd.DataFrame, gen_reg: pd.DataFrame, marginal_reg: pd.DataFrame) -> pd.DataFrame:
    tech_map = (
    ...
    reg.loc[
        reg["resolved_bmu"].isna() & reg["id_type"].isin(["structured_stack_id", "structured_other"]),
        "resolution_rule",
    ] = "structured_unresolved_manual_review"
...
def build_identity_substitute_candidates(ranked: pd.DataFrame, register: pd.DataFrame) -> pd.DataFrame:
    ...
    valid["identity_substitute_choice_rank"] = (
        valid.groupby(["settlementDate", "settlementPeriod", "direction"]).cumcount() + 1
    )
    ...
    return valid[cols].copy()


def apply_identity_fallback(
    action_resolved: pd.DataFrame,
    ranked: pd.DataFrame,
    ...
    out = out.merge(
        top_substitutes,
        on=["settlementDate", "settlementPeriod", "marginal_side_winner"],
        how="left",
    )
    ...
    out.loc[use_substitute, "marginal_generator_id_final"] = out.loc[
        use_substitute, "marginal_identity_substitute_generator_id"
    ]

    out["marginal_generator_label_final"] = out["marginal_generator_label_winner_resolved"]
    out.loc[use_substitute, "marginal_generator_label_final"] = out.loc[
        use_substitute, "marginal_identity_substitute_generator_label"
    ]
    ...
    out.loc[use_substitute, "marginal_identity_resolution_rule_final"] = (
        "numeric_winner_replaced_with_next_named_same_side"
    )
    out.loc[numeric_unresolved & ~has_substitute, "marginal_identity_resolution_rule_final"] = (
        "numeric_winner_unresolved_kept"
    )

    out["marginal_identity_price_basis"] = "raw_winner_price"
    out["marginal_identity_substitute_price_delta"] = (
        out["marginal_price_winner"] - out["marginal_identity_substitute_price"]
    )
    ...
    audit = audit[
        ...
        ["marginal_identity_substituted", "settlementDate", "settlementPeriod"],
...
def build_audit(register: pd.DataFrame) -> pd.DataFrame:
    audit = (
        ...
        .agg(
            n_ids=("raw_id", "size"),
            raw_rows=("raw_row_count", "sum"),
            winner_rows=("winner_sp_count", "sum"),
        )
        .reset_index()
...
def build_summary(register: pd.DataFrame, review: pd.DataFrame, audit: pd.DataFrame, action_resolved: pd.DataFrame) -> dict:
    return {
        ...
        "winner_rows_manual_review_share": float(
            register.loc[register["needs_manual_review"], "winner_sp_count"].sum() / register["winner_sp_count"].sum()
        ) if register["winner_sp_count"].sum() else 0.0,
        ...
        "winner_side_counts": action_resolved["marginal_side_winner"].value_counts(dropna=False).to_dict(),
        "top_unresolved_winner_ids": (
            ...
            [["raw_id", "id_type", "winner_sp_count", "raw_row_count"]]
            .head(25)
...
def main() -> None:
    ...
    summary = build_summary(register, review, audit, action_resolved)
    summary["winner_rows_identity_substituted"] = int(action_final["marginal_identity_substituted"].sum())
    summary["winner_rows_identity_substituted_share"] = float(action_final["marginal_identity_substituted"].mean())
    summary["numeric_winner_rows_unresolved_after_fallback"] = int(
        (
            action_final["marginal_winner_resolution_rule"].eq("numeric_raw_id_manual_review")
            & ~action_final["marginal_identity_substituted"]
        ).sum()
    )
    ...
    print("=" * 72)
    print("Q1 ISPSTACK ID Resolution Built")
    print("=" * 72)
    ...
    print(f"Register                  : {OUT_REGISTER}")
    print(f"Resolved action panel     : {OUT_ACTION_APPLIED}")
    print(f"Final identity panel      : {OUT_ACTION_FINAL}")
