...
INPUT_DIR = PROJECT_ROOT / "data" / "raw" / "ispstack" / "q1_2026"
DISBSAD_PATH = PROJECT_ROOT / "data" / "raw" / "ispstack" / "disbsad_q1_2026" / "disbsad_q1_2026.parquet"
BMU_REFERENCE_PATH = PROJECT_ROOT / "data" / "raw" / "reference" / "bmu_register_elexon_current.csv"
OUTPUT_DIR = PROJECT_ROOT / "data" / "processed" / "q1_2026" / "bid_offer_stack_q1_2026"
...
def load_q1() -> tuple[pd.DataFrame, int]:
    files = sorted(INPUT_DIR.glob("*.parquet"))
    files = [f for f in files if START_DATE <= pd.Timestamp(f.stem) <= END_DATE]
    if not files:
        raise RuntimeError(f"No parquet files found in {INPUT_DIR} for Q1 2026")
...
def main() -> None:
    ...
    print("Loading Q1 2026 ISPSTACK data ...")
    raw, n_files = load_q1()
    ...
    print(f"  DISBSAD rows : {len(disbsad_lookup):,}")
    ...
    out_parquet = OUTPUT_DIR / "bid_offer_stack_energy_q1_2026_long.parquet"
    stack.to_parquet(out_parquet, index=False)
    print(f"\nSaved -> {out_parquet}")
