...
BMU_MAP_PATH = OUT_DIR / "bid_offer_stack_energy_2023_2025_bmu_map.csv"
REGISTER_PATH = PROJECT_ROOT / "data" / "raw" / "reference" / "bmu_register_2023_2025.csv"
OUT_LONG = OUT_DIR / "bid_offer_stack_energy_2023_2025_niv_marginal.parquet"
OUT_SP = OUT_DIR / "bid_offer_stack_energy_2023_2025_niv_marginal_sp_summary.parquet"
...
def load_niv() -> pd.DataFrame:
    ...
    niv = niv[
        SP_KEYS + ["netImbalanceVolume", "systemLongShort", "systemBuyPrice", "systemSellPrice", "systemPrice"]
    ].copy()
...
def identify_marginal(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _flag_all_at_top(subset: pd.DataFrame, price_rule: str) -> None:
        if subset.empty:
            return
        if price_rule == "max":
            marginal_price = subset.groupby(SP_KEYS)["finalPrice"].transform("max")
        elif price_rule == "min":
            marginal_price = subset.groupby(SP_KEYS)["finalPrice"].transform("min")
        else:
            raise ValueError(f"Unknown marginal price rule: {price_rule}")
        df.loc[subset.index[subset["finalPrice"] == marginal_price], "is_niv_marginal"] = True

    _flag_all_at_top(
        df[(df["niv_active_side"] == "offer") & (df["side"] == "offer")].copy(),
        price_rule="max",
    )
    _flag_all_at_top(
...
def build_sp_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    marginal = marginal.rename(columns={
        "id":                          "marginal_bmu_raw",
    ...
    keep = SP_KEYS + [
        ...
        "marginal_bmu_raw", "marginal_bmu_id", "marginal_is_numeric_id", "marginal_identity_source",
        "marginal_fuel_type", "marginal_family_id", "marginal_lead_party", "marginal_bmu_type",
        "marginal_gen_capacity_mw", "marginal_gsp_group",
        "marginal_final_price", "marginal_original_price", "marginal_price_cost_to_so",
        "marginal_stack_rank", "marginal_bid_rank", "marginal_volume",
...
def main() -> None:
    print("Loading 2023-2025 bid-offer stack ...")
    ...
    print(f"  fuel_type coverage: {len(df) - n_fuel_null:,} / {len(df):,} rows")

    print("Joining family_id and named IC fuel_type from register ...")
    assert REGISTER_PATH.exists(), f"Register not found: {REGISTER_PATH} — run build_bmu_register_2023_2025.py first"
    reg = pd.read_csv(REGISTER_PATH)[["elexon_bmu_id", "family_id", "fuel_type"]]
    df = df.merge(reg.rename(columns={"fuel_type": "reg_fuel_type"}),
                  left_on="resolved_bmu_id", right_on="elexon_bmu_id", how="left").drop(columns="elexon_bmu_id")
    # Override BMU map fuel_type with register's named IC label where applicable
    ic_override = df["fuel_type"] == "INTERCONNECTOR"
    df.loc[ic_override, "fuel_type"] = df.loc[ic_override, "reg_fuel_type"].fillna("INTERCONNECTOR")
    df = df.drop(columns="reg_fuel_type")
    n_fam_null = df["family_id"].isna().sum()
    print(f"  family_id coverage: {len(df) - n_fam_null:,} / {len(df):,} rows")
    n_ic_named = (df["fuel_type"].isin(["IFA","IFA2","BritNed","NEMO","Eleclink","Viking","Moyle","EastWest","NSL","Greenlink"])).sum()
    print(f"  Named IC rows     : {n_ic_named:,}")
    ...
    print()
    print("Marginal fuel type breakdown:")
    print(sp_summary["marginal_fuel_type"].value_counts(dropna=False).rename("n_sps").to_string())
