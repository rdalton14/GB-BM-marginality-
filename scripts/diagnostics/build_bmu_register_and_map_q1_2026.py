...
OUT_MAP = (
    PROJECT_ROOT
    / "data" / "processed" / "q1_2026" / "bid_offer_stack_q1_2026"
    / "bid_offer_stack_energy_q1_2026_bmu_map.csv"
)
...
def load_fuel_typ() -> pd.DataFrame:
    ...
    df["reg_fuel_type"] = df["reg_fuel_type"].astype(str).str.strip().replace("nan", pd.NA)
...
def main() -> None:
    ...
    if "fuelType" in reg.columns:
        reg["fuelType"] = reg["fuelType"].astype(str).str.strip()
    reg = reg.drop_duplicates(subset=["elexonBmUnit"], keep="first")
