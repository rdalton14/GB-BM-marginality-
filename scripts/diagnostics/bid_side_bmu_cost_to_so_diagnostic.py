...
OUT_CSV  = REPORT_DIR / "tables"    / "bid_side_bmu_cost_to_so_diagnostic.csv"
OUT_HTML = REPORT_DIR / "canonical" / "bmu" / "bid_side_bmu_cost_to_so_diagnostic.html"

PREDICTOR_COL   = shap_bmu.PREDICTOR_COL
PREDICTOR_LABEL = shap_bmu.PREDICTOR_LABEL

TOP_N_BID_FEATURES = 35
COST_EPSILON       = 1e-9
...
def classify_bid_direction(so_pays_share: float, so_receives_share: float) -> str:
...
def build_shap_bid_table(sp: pd.DataFrame, fuel_lookup: dict[str, str]) -> pd.DataFrame:
    sided_col = f"{PREDICTOR_COL}_sided"
    sp = sp.copy()
    sp[sided_col] = shap_bmu.make_sided_col(sp, PREDICTOR_COL)
    ...
    shap_df = pd.DataFrame(
        {
            ...
            "fuel_type":                        [shap_bmu.feature_fuel(fn, fuel_lookup) for fn in feature_names],
...
def aggregate_bid_prices(sp: pd.DataFrame) -> pd.DataFrame:
    sided_col = f"{PREDICTOR_COL}_sided"
    bid = sp.loc[sp["niv_active_side"].eq("bid")].copy()
    bid[sided_col] = shap_bmu.make_sided_col(bid, PREDICTOR_COL)
    bid["marginal_final_price"] = pd.to_numeric(bid["marginal_final_price"], errors="coerce")
    ...
    for feature, grp in bid.groupby(sided_col, dropna=False):
        raw_bid = grp["marginal_final_price"]
        weights = grp["tie_weight"]
        so_pays_mask     = raw_bid.lt(-COST_EPSILON)
        so_receives_mask = raw_bid.gt(COST_EPSILON)
        zero_bid_mask    = raw_bid.abs().le(COST_EPSILON)
        ...
        rows.append(
            {
                ...
                "unique_settlement_periods":  int(grp[["settlementDate", "settlementPeriod"]].drop_duplicates().shape[0]),
                "median_system_price":        float(grp["systemPrice"].median()),
                "mean_raw_bid_price":         float(raw_bid.mean()),
                "median_raw_bid_price":       float(raw_bid.median()),
                "weighted_mean_raw_bid_price": weighted_mean(raw_bid, weights),
                "share_so_pays_weighted":     so_pays_share,
                "share_so_receives_weighted": so_receives_share,
                "share_zero_bid_weighted":    zero_bid_share,
                "raw_bid_price_direction":    classify_bid_direction(so_pays_share, so_receives_share),
            }
        )
    return pd.DataFrame(rows)


def plot_direction_mix(df: pd.DataFrame) -> str:
    block = df.sort_values("mean_abs_shap", ascending=True).tail(TOP_N_BID_FEATURES)
    labels = [
        f"{row['feature']}  [{row['fuel_type']}]"
        for _, row in block.iterrows()
    ]
    y = np.arange(len(block))
    fig, ax = plt.subplots(figsize=(10, max(5, 0.36 * len(block) + 1)))
    ax.barh(y, block["share_so_pays_weighted"],     color="#D32F2F", label="SO pays / negative bid")
    ax.barh(y, block["share_so_receives_weighted"], left=block["share_so_pays_weighted"],
            color="#2E7D32", label="SO receives / positive bid")
    ax.barh(y, block["share_zero_bid_weighted"],
            left=block["share_so_pays_weighted"] + block["share_so_receives_weighted"],
            color="#BDBDBD", label="Zero bid")
    ax.set_yticks(y)
    ax.set_yticklabels(labels, fontsize=8)
    ax.set_xlim(0, 1)
    ax.set_xlabel("Tie-weighted share of bid-side marginal rows")
    ax.set_title(
        f"Raw bid-price direction — top {TOP_N_BID_FEATURES} bid-side BMU SHAP drivers", fontsize=11
    )
...
img { display:block; max-width: 950px; width:100%; border-radius: 6px;
...
.callout { background:#F5F7FA; border-left:5px solid #2E7D32; padding:12px 16px; margin:18px 0 22px 0; }
...
def make_html(df: pd.DataFrame, sp: pd.DataFrame) -> str:
    plot_b64 = plot_direction_mix(df)
    direction_counts = df["raw_bid_price_direction"].value_counts().to_dict()
    stat_html = "".join(
        f'<span class="stat"><strong>{v}</strong>{k}</span>'
        for k, v in direction_counts.items()
    )
    show_cols = [
        "bid_shap_rank", "overall_shap_rank", "feature", "fuel_type",
        "mean_abs_shap", "bid_rows", "weighted_bid_rows",
        "median_system_price", "median_raw_bid_price",
        "weighted_mean_raw_bid_price",
        "share_so_pays_weighted", "share_so_receives_weighted",
        "share_zero_bid_weighted", "raw_bid_price_direction",
    ]
    bid_rows = sp.loc[sp["niv_active_side"].eq("bid")]
    return f"""<!DOCTYPE html>
<html><head><meta charset="utf-8">
<title>BMU Bid-side cost-to-SO diagnostic</title>
{CSS}
</head><body>
<h1>BMU ID — Main-Model BID Drivers: Raw Bid-Price Direction</h1>
<div class="callout">
  <p><strong>Canonical bid-side accounting interpretation at BMU level.</strong> Starts from
  the main BMU SHAP price model's top <code>_BID</code> features, then checks whether those
  marginal bid rows are predominantly negative bids where the SO pays, or positive bids where
  the SO receives. Fuel type is annotated alongside each BMU.</p>
</div>
<p class="note">SHAP columns determine which BID-side BMUs matter in the main price model.
The direction chart and table then focus on raw bid-price sign: negative raw bid prices =
SO pays; positive = SO receives. Shares are tie-weighted using
...
<table>
  ...
  <tr><td>Top N bid features shown</td><td>{TOP_N_BID_FEATURES}</td></tr>
</table>
<h2>Raw bid-price sign mix</h2>
<div>{stat_html}</div>
<img src="data:image/png;base64,{plot_b64}">
<h2>BMU bid-side detail table</h2>
{df[show_cols].round(4).to_html(index=False, border=0)}
</body></html>"""
...
def main() -> None:
    ...
    print("\nBMU bid-side: fitting SHAP model and aggregating raw bid prices ...")
    shap_bid    = build_shap_bid_table(sp, fuel_lookup)
    price_stats = aggregate_bid_prices(sp)
    df          = shap_bid.merge(price_stats, on="feature", how="left")

    print("  top bid BMUs: " + ", ".join(df["feature"].head(5).astype(str).tolist()))
