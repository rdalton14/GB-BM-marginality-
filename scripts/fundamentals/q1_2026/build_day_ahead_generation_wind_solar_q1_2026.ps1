...
$ProcessedCsv = Join-Path $ProcessedDir "day_ahead_generation_wind_solar_q1_2026.csv"
$DailyCompletenessCsv = Join-Path $DiagnosticsCoverageDir "day_ahead_generation_wind_solar_q1_2026_daily_completeness.csv"
$MissingSpsCsv = Join-Path $DiagnosticsMissingnessDir "day_ahead_generation_wind_solar_q1_2026_missing_sps.csv"
$RawProcessTypeCsv = Join-Path $DiagnosticsCoverageDir "day_ahead_generation_wind_solar_q1_2026_raw_process_type_counts.csv"
...
while ($day -le $EndDate) {
    ...
    $processTypes = @($rows | Select-Object -ExpandProperty processType -Unique)
    foreach ($processType in $processTypes) {
        $count = @($rows | Where-Object { $_.processType -eq $processType }).Count
...
$rawProcessTypeCounts | Sort-Object settlementDate, processType | Export-Csv -NoTypeInformation -Path $RawProcessTypeCsv
...
Write-Host "raw process-type counts path = $RawProcessTypeCsv"
