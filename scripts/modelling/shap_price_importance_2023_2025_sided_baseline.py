...
OUT_HTML = (
    ...
    / "shap_price_importance_2023_2025_sided_baseline.html"
)
...
N_TREES = {
    ...
    "marginal_family_id": 100,
    "marginal_bmu_id":    100,
}
MAX_DEPTH = {
    "marginal_fuel_type": 10,
    "marginal_family_id": 12,
    "marginal_bmu_id":    12,
}
MIN_SAMPLES_LEAF = {
    "marginal_fuel_type": 25,
    "marginal_family_id": 25,
    "marginal_bmu_id":    25,
}
SHAP_MAX = {
    "marginal_fuel_type": 5_000,
    "marginal_family_id":  2_000,
    "marginal_bmu_id":     1_000,
}
...
<title>SHAP Price Importance (Sided Baseline) — 2023–2025</title>
{CSS}
</head><body>
<h1>SHAP Baseline: System Price ~ Marginal Identity × NIV Side — 2023–2025</h1>
<p>Baseline TreeSHAP values from one conservative RF regressor per predictor family (seed=0).
...
<section>
  ...
  <table class="tbl">
    ...
    <tr><td>Profile</td><td>Baseline: conservative depth/leaf settings used as the stable reference case.</td></tr>
  ...
  <p class="note">SHAP decomposes each prediction into additive per-feature contributions around a global
     baseline (expected model output). For binary dummy features, the SHAP value when feature=1
     captures how much that specific label×side combination moves the predicted price relative to
     the average prediction — accounting for all interaction effects learned by the forest.</p>
</section>"""
