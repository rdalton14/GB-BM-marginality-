...
OUT_HTML = PROJECT_ROOT / "reports" / "q1_2026" / "canonical" / "rf_price_importance_q1_2026.html"
...
def plot_price_by_category(sp: pd.DataFrame, col: str, label: str, n_top: int = TOP_N) -> str:
    """Box plot of systemPrice grouped by category (top N by median)."""
  ...
  <table class="tbl">
    ...
    for col, label in PREDICTORS:
        ...
        stat_html = (
            ...
            f'<span class="stat"><strong>{feature_names.__len__()}</strong>one-hot features</span>'
...
<title>RF Price Importance — Q1 2026</title>
{CSS}
</head><body>
<h1>RF Feature Importance: System Price ~ Marginal Identity — Q1 2026</h1>
